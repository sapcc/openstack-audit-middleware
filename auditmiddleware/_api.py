...
class OpenStackAuditMiddleware(object):
    ...
    def _build_audit_map(self, res_dict, parent_type_uri=None):
        """Creates the mapping from REST resource names to the resource descriptor

        The descriptor contains all the information needed to produce the CADF events
        from HTTP requests."""
        result = {}
    ...
    def _build_events(self, target_project, res_spec, res_id, res_parent_id,
        ...
        hierarchy from it. The res_spec resource tree is used to interpret the path segments properly,
        e.g. known when a path segment represents a resource name, an ID or an attribute name.
